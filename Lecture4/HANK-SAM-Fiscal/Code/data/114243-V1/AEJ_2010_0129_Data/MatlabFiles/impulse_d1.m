...
% and allows for dummy at end of X and lags of dummy

function irf = impulse_d1(Y,X,periods,shockpos,levelindex,dummylags);

[T,K]=size(X);
beta=inv(X'*X)*(X'*Y);
res=Y-X*beta;
X=X(:,1:K-dummylags); beta=beta(1:K-dummylags); %%%% This is part that removes dummy effect
