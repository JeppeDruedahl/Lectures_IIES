...
function [p1, P]=AICsimFIT1(Y,X,N,lagAIC,ARlags,levelindex,periods,ARvec,MAvec,minindex);
...
% P is estimated fitted value for each lag specification for different true
% lag specifications.
...
for it=1:N
    ...
    % estimate fitted value given correct specification
    fitt(it,:)=fit1(Y1(:,1),[ones(length(Y1),1) Y1(:,2:1+ARlags) mp1(:,2:1+MAlags)],periods,ARlags+2,levelindex);
    ...
    for j1=1:length(ARvec)
        ...
        for k1=1:length(MAvec)
            ...
            fitA(it,:,ind)=fit1(Y1(:,1),[ones(length(Y1),1) Y1(:,2:1+j) mp1(:,2:1+k)],periods,j+2,levelindex);
            ind=ind+1;
        end
    end
