...
% THIS FILE ALLOWS FOR A CONTROL VARIABLE (COMMODITIES) ON RHS

function irf = impulseCOM(Y,X,periods,shockpos,levelindex,lagsControl);

[T,K]=size(X);
beta=inv(X'*X)*(X'*Y);
res=Y-X*beta;

beta=beta(1:length(beta)-lagsControl-1);
